...
class ScalaLanguageServer(SolidLanguageServer):
    ...
    def _setup_runtime_dependencies(cls, config: LanguageServerConfig, solidlsp_settings: SolidLSPSettings) -> list[str]:
        ...
        """
        java_path = find_executable_in_path("java")
        assert java_path is not None, "JDK is not installed or not in PATH."
        ...
        # Keep versioned installs to avoid clobbering across upgrades.
        metals_executable = os.path.join(metals_home, metals_version, "metals")

        # Use Windows-aware PATH lookup for both coursier and cs.
        coursier_command_path = find_executable_in_path("coursier")
        cs_command_path = find_executable_in_path("cs")
        ...
        if not os.path.exists(metals_executable):
            ...
            os.makedirs(os.path.join(metals_home, metals_version), exist_ok=True)
            ...
            subprocess.run(cmd, cwd=metals_home, check=True, env=os.environ.copy())
