...
fn count_symbols(symbols: &[MockSymbol]) -> usize {
    symbols.iter().fold(0, |acc, s| {
        acc + 1 + count_symbols(&s.children)
    })
}

/// Find symbol by name path (e.g., "Class/method")
fn find_symbol_by_path<'a>(
    symbols: &'a [MockSymbol],
    path: &[&str],
) -> Option<&'a MockSymbol> {
    if path.is_empty() {
        return None;
...
fn bench_symbol_lookup(c: &mut Criterion) {
    ...
    for path in paths {
        ...
        group.bench_with_input(
            BenchmarkId::new("find", &path_str),
            &path,
            |b, path| {
                b.iter(|| {
                    find_symbol_by_path(black_box(&tree), black_box(&path[..]))
                });
            },
        );
    }
...
fn bench_symbol_cache(c: &mut Criterion) {
    ...
    group.bench_function("cache_lookup_hit", |b| {
        b.iter(|| {
            cache.get(black_box("file_50.rs"))
        });
    });

    group.bench_function("cache_lookup_miss", |b| {
        b.iter(|| {
            cache.get(black_box("nonexistent.rs"))
        });
    });
...
fn bench_content_processing(c: &mut Criterion) {
    ...
    let large_content: String = (0..10000)
        .map(|i| format!("line {} with some content and more text to make it longer\n", i))
        .collect();

    group.throughput(Throughput::Bytes(small_content.len() as u64));
    group.bench_function("count_lines_small", |b| {
        b.iter(|| {
            black_box(&small_content).lines().count()
        });
    });
    ...
    group.bench_function("count_lines_large", |b| {
        b.iter(|| {
            black_box(&large_content).lines().count()
        });
    });
    ...
    group.bench_function("pattern_match_small", |b| {
        b.iter(|| {
            pattern.find_iter(black_box(&small_content)).count()
        });
    });

    group.bench_function("pattern_match_large", |b| {
        b.iter(|| {
            pattern.find_iter(black_box(&large_content)).count()
        });
    });
...
fn bench_reference_tracking(c: &mut Criterion) {
    ...
    group.bench_function("find_references", |b| {
        b.iter(|| {
            references.get(black_box("Symbol_500"))
        });
    });

    group.bench_function("collect_all_refs", |b| {
        b.iter(|| {
            references.values().flatten().count()
        });
    });
