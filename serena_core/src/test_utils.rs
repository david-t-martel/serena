//! Test Utilities for Serena Core
//!
...
//!     mock_server.add_tool("find_symbol", json!({"type": "object"}));
//!     let response = mock_server.call("tools/list").await;
...
macro_rules! assert_contains_tool {
    ($tools:expr, $name:expr) => {
        assert!(
            $tools.iter().any(|t| t.get("name").and_then(|v| v.as_str()) == Some($name)),
            "Tool '{}' not found in {:?}",
            $name,
            $tools
        );
...
mod tests {
    ...
    #[test]
    fn test_test_project_creation() -> std::io::Result<()> {
        let project = TestProject::python()?;

        assert!(project.path().join("src/main.py").exists());
        assert!(project.path().join(".serena/project.yml").exists());

        let content = project.read_file("src/main.py")?;
        assert!(content.contains("class MyClass"));

        Ok(())
    }

    #[test]
    fn test_test_project_write_file() -> std::io::Result<()> {
        let project = TestProject::new()?;

        project.write_file("src/new_file.py", "# New file\n")?;

        assert!(project.path().join("src/new_file.py").exists());
        assert_eq!(project.read_file("src/new_file.py")?, "# New file\n");
