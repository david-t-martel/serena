...
use super::tools::{SerenaTools, FileService, SymbolService, MemoryService};
...
impl SerenaServerHandler {
    pub fn new(args: &CommandArguments) -> SerenaResult<Self> {
        let project_root = args.project_path.clone()
...
impl ServerHandler for SerenaServerHandler {
    ...
    async fn handle_call_tool_request(
        ...
        match tool_params {
            // File Tools
            SerenaTools::ReadFile(params) => {
                params.run_tool(&self.file_service).await
            }
            SerenaTools::CreateTextFile(params) => {
                params.run_tool(&self.file_service).await
            }
            SerenaTools::ListDir(params) => {
                params.run_tool(&self.file_service).await
            }
            SerenaTools::FindFile(params) => {
                params.run_tool(&self.file_service).await
            }
            SerenaTools::ReplaceContent(params) => {
                params.run_tool(&self.file_service).await
            }
            SerenaTools::SearchForPattern(params) => {
                params.run_tool(&self.file_service).await
            }

            // Symbol Tools
            SerenaTools::GetSymbolsOverview(params) => {
                params.run_tool(&self.symbol_service).await
            }
            SerenaTools::FindSymbol(params) => {
                params.run_tool(&self.symbol_service).await
            }
            SerenaTools::FindReferencingSymbols(params) => {
                params.run_tool(&self.symbol_service).await
            }
            SerenaTools::ReplaceSymbolBody(params) => {
                params.run_tool(&self.symbol_service).await
            }
            SerenaTools::RenameSymbol(params) => {
                params.run_tool(&self.symbol_service).await
            }

            // Memory Tools
            SerenaTools::WriteMemory(params) => {
                params.run_tool(&self.memory_service).await
            }
            SerenaTools::ReadMemory(params) => {
                params.run_tool(&self.memory_service).await
            }
            SerenaTools::ListMemories(params) => {
                params.run_tool(&self.memory_service).await
            }
            SerenaTools::DeleteMemory(params) => {
                params.run_tool(&self.memory_service).await
            }
            SerenaTools::EditMemory(params) => {
                params.run_tool(&self.memory_service).await
            }
        }
    }
